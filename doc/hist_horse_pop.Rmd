...
The R programming language and the following R packages were used to perform the analysis: knitr and tidyverse [@tidyverse]. The code used to perform the analysis and create this report can be found here: <https://github.com/ttimbers/equine_numbers_value_canada_rmd>.

## Results

```{r plot horses, fig.cap = "Figure 1: The historical number of horses has seen the greatest change in primarily more prairie provinces such as Saskatchewan, Alberta and Ontario.",  fig.width=10, fig.height=3}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
kable(max_horses, caption="Table 1: Horse population per province.")
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.cap= paste("Figure 2: The change in horse population for ", params$province), fig.width=5, fig.height=4}
